...
export function getElem(id: string) {
...
export function phaseToId(ph: number) {
...
export function neg(id: string) {
  const a = getElem(id);
  const ph = ((Math.PI / 2 - a.phase) % TAU + TAU) % TAU;
  return phaseToId(ph);
}

export function conj(idA: string, idB: string) {
  const a = getElem(idA);
  const b = getElem(idB);
  return phaseToId(a.phase + b.phase);
}

export function disj(idA: string, idB: string) {
  const a = getElem(idA);
  const b = getElem(idB);
  return phaseToId(a.phase + b.phase - Math.PI / 2);
}

export function impl(idA: string, idB: string) {
  const a = getElem(idA);
  const b = getElem(idB);
